...
def test_chroma_store_and_retrieve(tmp_path, monkeypatch):

    monkeypatch.setattr(
        "app.vector_stores.chroma.CHROMA_DB_PATH",
        str(tmp_path)
    )

    monkeypatch.setattr(
        "app.vector_stores.chroma.CHROMA_COLLECTION_NAME",
        "test_collection"
    )
...
def test_chroma_metadata_filter(tmp_path, monkeypatch):
